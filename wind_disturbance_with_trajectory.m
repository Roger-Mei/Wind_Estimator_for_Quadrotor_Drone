...
X_dot = zeros(12,1);
% Generate target trajectory (world frame 1)
pf = [34;34;34]
v0 = zeros(3,1);
a0 = zeros(3,1);
time_horizon = tspan(2);
sample_time = dt;
...
% Set up the trajectory saver
trajectory_saver = zeros(tspan(2)/dt+1, 3);
% Generate wind disturbance model
n = 5;
L_v = 3; 
sigma_v = 2.5;
d_w = wind_simulation(tspan, dt, n, L_v, sigma_v);
...
%     u_w = 1.2*sin(2*t); % x-direction
%     v_w = 1.2*sin(1.5*t); % y-direction
%     w_w = 0.5*sin(0.2*t); % z-direction
    u_w = 0.65*real(d_w(1,index));
    v_w = 0.5*real(d_w(2,index));
    w_w = real(d_w(3,index));
    d_w_W1 = [u_w; v_w; w_w];
    ...
    % frame
    d_w_B2 = R_B2_W2'*d_w_W2;
    d_w_est_B2 = R_B2_W2'*d_w_est_W2;
    %% ----------------------Calculate inputs----------------------------%%
    ...
    acl_B2 = -cross([p;q;r], R_B2_W2'*X(7:9)) + F_aero/P.m + R_B2_W2'*[0;0;P.g];
    ...
    %% -------------------Construct the trajectory saver-----------------%%
    % Calculate position in world frame 
    X_W1 = R_21*X(1:3);
    trajectory_saver(index,1) = X_W1(1);
    trajectory_saver(index,2) = X_W1(2);
    trajectory_saver(index,3) = X_W1(3);
end
%% --------------------------Plot out the data --------------------------%%
% Plot the x-axis wind
figure('Name', 'Estimated wind');
subplot(6,1,1);
x = data_saver(:,1);
y1_est = data_saver(:,2);
y1_act = data_saver(:,5);
...
% Actual trajectory
x_act = trajectory_saver(:,1);
y_act = trajectory_saver(:,2);
z_act = trajectory_saver(:,3);
% Target trajectory
x_t = Z(1,:);
y_t = Z(2,:);
z_t = Z(3,:);
plot3(x_act, y_act, z_act, 'r', 'LineWidth', 2.5);
hold on;
plot3(x_t, y_t, z_t, 'b', 'LineWidth', 2.5);
grid on;
